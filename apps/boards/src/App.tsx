...
import type { KanbanBoard, KanbanBoardSnapshot, KanbanTicket, KanbanUser } from "@opendock/shared/types";
...
function TicketCard({
  ...
  return (
    <div
      className={clsx(
        "group relative flex flex-col gap-2 rounded-lg border border-neutral-200 bg-white p-3 pl-5 text-neutral-700 transition-all hover:border-neutral-300 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-200",
...
function SortableTicket({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={clsx("cursor-grab select-none active:cursor-grabbing", isDragging && "opacity-80")}
      {...attributes}
      {...listeners}
    >
      <TicketCard
...
function KanbanColumn({
  ...
  return (
    <div className="flex min-w-[22rem] max-w-[22rem] flex-col gap-3 rounded-lg border border-neutral-200 bg-white p-4 transition dark:border-neutral-800 dark:bg-neutral-950">
      ...
      <SortableContext id={column.id} items={tickets.map((ticket) => ticket.id)} strategy={verticalListSortingStrategy}>
        <div
          ref={setNodeRef}
          className={clsx(
            "flex min-h-[6rem] flex-col gap-2.5 rounded-md border border-transparent p-1",
...
function BoardsAppInner() {
  ...
  const [boardForm, setBoardForm] = useState({ name: "", description: "", members: "" });
                  ...
                  <div className="overflow-x-auto">
                    <div className="mx-auto flex w-max gap-5 px-4 pb-6 sm:px-6 lg:px-10">
                      ...
                      <form
                        ...
                        className="flex min-w-[20rem] max-w-[20rem] flex-col gap-3 rounded-lg border border-dashed border-slate-300/70 bg-white/60 p-5 text-sm text-slate-500 shadow-sm dark:border-white/20 dark:bg-white/10"
                      >
