    ...
    var body: some View {
        VStack(spacing: 0) {
            TextField("Untitled", text: $title)
                ...
                .onChange(of: title) { _, v in store.update(noteId, title: v) }

            if let note, !note.tags.isEmpty {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) { ForEach(note.tags, id: \.self) { Text($0).font(.custom(Theme.fontName, size: 11)).foregroundColor(Theme.faint) } }
            ...
            TextEditor(text: $content)
                .font(.custom("Menlo", size: 14)).foregroundColor(Theme.text)
                .scrollContentBackground(.hidden).padding(.horizontal, 16).padding(.top, 8)
                .onChange(of: content) { _, v in store.update(noteId, content: v) }

            HStack(spacing: 8) {
                Text("\(note?.wordCount ?? 0) words"); Text("·"); Text("saved"); Spacer()
            }
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    Divider()
                    Button(role: .destructive) { store.delete(noteId); dismiss() } label: { Label("Delete", systemImage: "trash") }
