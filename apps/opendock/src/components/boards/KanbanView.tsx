...
export function KanbanView({ snapshot, onTicketClick, totalTicketCounts }: KanbanViewProps) {
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="flex h-full gap-4 overflow-x-auto pb-4 scroll-smooth select-none px-5 lg:px-8">
        {sortedColumns.map((column) => {
          const colTickets = getColumnTickets(column.id);
          return (
            <Droppable key={column.id} droppableId={column.id}>
              {(provided, snap) => (
                <KanbanColumn column={column} ticketCount={colTickets.length}
                  totalCount={totalTicketCounts?.get(column.id)}
                  droppableProvided={provided} isDraggingOver={snap.isDraggingOver}
                  footer={<QuickCreateTicket onCreateTicket={async (title) => { await createTicket(board.id, column.id, title); }} />}>
                  {colTickets.map((ticket, index) => (
                    <SortableTicketCard key={ticket.id} ticket={ticket} index={index} labels={labels}
                      members={members} onClick={() => onTicketClick(ticket)} onTitleUpdate={handleTitleUpdate} />
                  ))}
                </KanbanColumn>
              )}
            </Droppable>
