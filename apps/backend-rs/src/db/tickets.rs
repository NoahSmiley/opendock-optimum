...
pub async fn create_ticket(
    ...
    sqlx::query_as::<_, TicketRow>(
        r#"INSERT INTO kanban_tickets
           (id, key, board_id, column_id, title, description, assignee_ids, tags, label_ids,
...
pub async fn update_ticket(
    ...
    let e = match get_ticket(pool, id).await? { Some(e) => e, None => return Ok(None) };
    let now = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Millis, true);
    ...
    let est = estimate.unwrap_or(e.estimate);
    let sp = story_points.unwrap_or(e.story_points);
    let pri = priority.unwrap_or(&e.priority);
    let si = sprint_id.unwrap_or(e.sprint_id.as_deref());
    let dd = due_date.unwrap_or(e.due_date.as_deref());
    sqlx::query_as::<_, TicketRow>(
...
pub async fn ticket_count_for_board(pool: &SqlitePool, board_id: &str) -> Result<i64, sqlx::Error> {
    let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM kanban_tickets WHERE board_id = ?")
        .bind(board_id)
        .fetch_one(pool)
        .await?;
    Ok(row.0)
